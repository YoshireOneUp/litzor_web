...
function verificar_sesion_activa() {
    if (!isset($_SESSION['id_cl']) || !isset($_SESSION['tipo_usuario'])) {
        header('Location: ../public/login.php?error=' . urlencode('Debes iniciar sesión'));
        exit;
    }
}
...
function verificar_organizador() {
    ...
    if ($_SESSION['tipo_usuario'] != 1) {
        header('Location: ../public/login.php?error=' . urlencode('Acceso denegado: Solo organizadores'));
        exit;
    }
}
...
function verificar_administrador() {
    ...
    if ($_SESSION['tipo_usuario'] != 2) {
        header('Location: ../public/login.php?error=' . urlencode('Acceso denegado: Solo administradores'));
        exit;
    }
}
?>